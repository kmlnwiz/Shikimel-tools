...
function output(array) {
    ...
    array.forEach((item, i) => {
        ...
        for (let j = 0; j < str_len; j++) {
            ...
            html += `<div id="str_b${i}-${j + 1}" class="str-blind blind-2 col py-4 m-0 border bg-white text-dark fw-bold text-center touch-none rounded-3 d-none border-secondary" style="font-size:5.0rem;">${item.question[j]}<br><small class="fw-nomal fs-3 opacity-50 fw-bold position-absolute text-center" style="width:2.80rem; margin-left: -1.4rem !important; margin-top: -0.75rem !important; pointer-events:none;">${j + 1}</small></div></div>`;
        }
        count += str_len;
    });
